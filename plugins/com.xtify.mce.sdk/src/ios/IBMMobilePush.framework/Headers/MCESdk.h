...
/** Manually initialize SDK, useful to wait until an event occurs before beginning to interact with the IBM servers. For instance, you might not want to create a userid/channelid until after a user logs into your system. Requires  autoInitialize=FALSE MceConfig.plist flag.
 */
 ...
 */
- (void)presentDynamicCategoryNotification:(NSDictionary *)userInfo fetchCompletionHandler:(void (^)(UIBackgroundFetchResult))completionHandler;
 ...
 */
- (void)processDynamicCategoryNotification:(NSDictionary *)userInfo identifier:(NSString*)identifier fetchCompletionHandler:(void (^)(UIBackgroundFetchResult))completionHandler;
 ...
 */
- (void)processCategoryNotification:(NSDictionary *)userInfo identifier:(NSString*)identifier fetchCompletionHandler:(void (^)(void))completionHandler;

/** Present or perform action in payload depending on the activation state of the application. If it was running before the message came in, present the notification. If it just started as a result of a notification then execute the notification-action part of the payload.
